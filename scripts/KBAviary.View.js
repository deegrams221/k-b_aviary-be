...
KBAviary.prototype.initTemplates = function () {
  this.templates = {};
...
KBAviary.prototype.viewHome = function () {
  this.getAllbirds();
};

KBAviary.prototype.viewList = function (filters, filter_description) {
  ...
  var renderer = {
    ...
    display: function (doc) {
      var data = doc.data();
      data['.id'] = doc.id;
      data['go_to_bird'] = function () {
        that.router.navigate('/birds/' + doc.id);
      };

      var el = that.renderTemplate('bird-card', data);
      el.querySelector('.rating').append(that.renderRating(data.avgRating));
      el.querySelector('.price').append(that.renderPrice(data.price));
      // Setting the id allows to locating the individual bird card
...
KBAviary.prototype.viewSetup = function () {
  var headerEl = this.renderTemplate('header-base', {
    hasSectionHeader: false,
  });

  var config = this.getFirebaseConfig();
  var nobirdsEl = this.renderTemplate('no-birds', config);

  var button = nobirdsEl.querySelector('#add_mock_data');
  ...
  button.addEventListener('click', function (event) {
    ...
    that.addMockbirds().then(function () {
      that.rerender();
    });
  });

  this.replaceElement(document.querySelector('.header'), headerEl);
  this.replaceElement(document.querySelector('main'), nobirdsEl);
...
KBAviary.prototype.initReviewDialog = function () {
...
KBAviary.prototype.initFilterDialog = function () {
...
KBAviary.prototype.updateQuery = function (filters) {
  var query_description = '';

  if (filters.category !== '') {
    query_description += filters.category + ' places';
  } else {
    query_description += 'any bird';
  }
...
KBAviary.prototype.viewbird = function (id) {
  var sectionHeaderEl;

  var that = this;
  return this.getbird(id)
    .then(function (doc) {
      var data = doc.data();
      ...
      sectionHeaderEl = that.renderTemplate('bird-header', data);
      sectionHeaderEl
...
KBAviary.prototype.renderTemplate = function (id, data) {
...
KBAviary.prototype.render = function (el, data) {
  if (!data) {
    return;
  }
...
KBAviary.prototype.useModifier = function (el, selector, modifier) {
  el.querySelectorAll('[' + selector + ']').forEach(modifier);
};

KBAviary.prototype.getDeepItem = function (obj, path) {
...
KBAviary.prototype.renderRating = function (rating) {
...
KBAviary.prototype.renderPrice = function (price) {
...
KBAviary.prototype.replaceElement = function (parent, content) {
  parent.innerHTML = '';
  parent.append(content);
};

KBAviary.prototype.rerender = function () {
